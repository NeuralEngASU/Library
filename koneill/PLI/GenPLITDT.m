...
sizeFile = round(sizeFile.bytes / 1024^2);
...
for ii = params.chanProcess
    
    varName = ['C', num2str(ii)];
    load(filePath, varName);    
    eval(['data(', num2str(ii), ',:) = ', varName, ';']);
    clear(varName);
...
rawWin = zeros(winNum, floor(winSize*Fs), numChans, numTrials);
...
rawWin = permute(reshape(data(1:floor(winNum*floor(winSize*Fs)),:,:), floor(winSize * Fs), winNum, numChans, numTrials), [2,1,3,4]);
