...
% loop over the grids
load('D:\Kevin\SpencerPaper\201203\g1mcoh_UNR.mat','f');
hhd=zeros(length(f),length(grids));
for g=1:length(grids)
    d=load(['D:\Kevin\SpencerPaper\201203\g' num2str(g) 'mcoh_' upper(ref) '.mat'],'mcoh','f','chanpairs','layout','spacing');
...
saveas(gcf,['D:\Kevin\SpencerPaper\201203\figures\MCOHHHD_' upper(ref) '.fig']);
saveas(gcf,['D:\Kevin\SpencerPaper\201203\figures\MCOHHHD_' upper(ref) '.png']);
saveas(gcf,['D:\Kevin\SpencerPaper\201203\figures\MCOHHHD_' upper(ref) '.eps'],'epsc2');
