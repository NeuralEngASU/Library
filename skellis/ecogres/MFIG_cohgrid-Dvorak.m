...
for g=1:length(grids)
    load(['d:/Spencer/ecogres/g' num2str(g) 'mcoh_' upper(ref) '.mat']);
    ...
    for q=1:length(freq.names)
        ...
        saveas(gcf,['d:/Spencer/ecogres/figures/MCHGRID_' grids(g).shortname '_' freq.names{q} '_' upper(ref) '.fig']);
        saveas(gcf,['d:/Spencer/ecogres/figures/MCHGRID_' grids(g).shortname '_' freq.names{q} '_' upper(ref) '.png']);
        saveas(gcf,['d:/Spencer/ecogres/figures/MCHGRID_' grids(g).shortname '_' freq.names{q} '_' upper(ref) '.eps'],'epsc2');
        close(gcf);
    end
end