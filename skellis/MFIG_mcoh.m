...
for g=1:length(grids)
    load(['D:\Kevin\SpencerPaper\201203\g' num2str(g) 'mcoh_' upper(ref) '.mat'],'mcoh','f','chanpairs','layout','spacing');
...
% % % % % % %         '75 mm','',' ',' ',' ','',' ',' ','','','','','','','','','100 mm';};
% % % % % % %     
% % % % % % %     lcolorbar(llegstr2,'Parent',ax(3));
% % % % % % %     %%%%%
    ...
    for k=1:length(fnames)
        plot(seps,nanmean(mcoh_no60(f>=fbands(k,1)&f<fbands(k,2),:)),'LineWidth',2);
    ...
    saveas(gcf,['D:\Kevin\SpencerPaper\201203\figures\MCOH_' grids(g).shortname '_' upper(ref) '.fig']);
    saveas(gcf,['D:\Kevin\SpencerPaper\201203\figures\MCOH_' grids(g).shortname '_' upper(ref) '.png']);
    saveas(gcf,['D:\Kevin\SpencerPaper\201203\figures\MCOH_' grids(g).shortname '_' upper(ref) '.eps'],'epsc2');
